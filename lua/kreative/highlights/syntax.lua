...
local colors, groups, main, run, ucolors = autoload("kreative.color"), autoload("kreative.highlights.main"), autoload("kreative.main"), autoload("kreative.utils.highlight.run"), autoload("kreative.utils.highlight.utils")
...
local function high_colors()
  local function _1_()
    local hi_group = {group = "Comment", fg = groups.meldBG()[1], bg = "SKIP", ctermfg = groups.selectionBG()[3], ctermbg = "SKIP"}
    for _, v in ipairs(main.configs.comment_style) do
      hi_group[v] = true
    end
    return hi_group
  end
