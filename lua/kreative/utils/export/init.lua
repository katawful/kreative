local _2afile_2a = "fnl/kreative/utils/export/init.fnl"
local _2amodule_name_2a = "kreative.utils.export.init"
local _2amodule_2a
do
...
local terminals = {"kitty", "alacritty", "konsole", "urxvt", "rxvt-unicode"}
_2amodule_2a["terminals"] = terminals
...
local function table__3eone_line_color(colors, terminal)
  local color_string = string.format(file_header, comment_type[1], terminal, comment_type[1], tostring(main.configs["colors-name"]), tostring(main.configs.background))
...
local function string__3eone_line_color(colors, terminal)
  ...
  local function _6_()
    local tbl = {}
    table.insert(tbl, string.format(file_header, comment_type[1], terminal, comment_type[1], tostring(main.configs["colors-name"]), tostring(main.configs.background)))
    for key, val in pairs(colors) do
...
local function notify_24(terminal)
  return message["info$"](string.format(message["<-table"]("utils.export.init", "term-theme-generated"), terminal, main.configs["colors-name"], main.configs.background))
end
_2amodule_2a["notify$"] = notify_24
local function is_colorscheme_3f()
  if (main.configs["colors-name"] ~= main.configs.colors_name) then
...
local function generate_color_files()
  local background = {"light", "dark"}
  local old_background = main.configs.background
  for _0, back in ipairs(backgrounds) do
    main.configs.background = back
    json["->file!"](string.format("%s/colors/%s-%s.json", json.path, main.configs.colors_name, back), json["encode-simple"](__fnl_global__color_2dtable.output()))
  end
  main.configs.background = old_background
  return nil
end
_2amodule_2a["generate-color-files"] = generate_color_files
...
local function gen_term_colors(terminal)
  if __fnl_global__all_3f then
    local _11_ = __fnl_global__all_3f
    if (_11_ == "all") then
      local backgrounds = {"light", "dark"}
      local old_background = main.configs.background
      for _0, v in ipairs(colors) do
        main.configs.background = v
        __fnl_global__color_2dtable.update()
    ...
    elseif true then
      local _0 = _11_
      return message["error$"](string.format(message["<-table"]("utils.export.init", "invalid-arg"), __fnl_global__all_3f))
    else
      return nil
    end
  else
...
vim.api.nvim_create_user_command("KreativeGenTermTheme", _24_, {nargs = "+", complete = command_completion})
return _2amodule_2a